...
"""
def retrive_AUTO_INCREMENT_ID_FromDB(dbh, auto_inc_service_type_id, table_name, column_1_name, column_1_value, column_2_name, column_2_value, column_3_name, column_3_value):
	
		
	try:
		#sql ="SELECT * FROM %s" % (tableName)
		sql ="SELECT %s FROM %s WHERE %s=%s AND %s=%s AND %s=%s" % (auto_inc_service_type_id, table_name, column_1_name, column_1_value,column_2_name,column_2_value , column_3_name, column_3_value)
		
		#sql ="SELECT * FROM schedule WHERE schedule_id=3"	
		cur = dbh.cursor()
		cur.execute(sql)
	
		data_frame = cur.fetchall() # fetch1
	except:
		data_frame = []
		errorMsg = "ERROR ERROR your retrive_AUTO_INCREMENT_ID_FromDB() function is not working"
		data_frame.append(errorMsg)

	return data_frame
#########################################
def retrive_AUTO_INCREMENT_ID_FromDB(dbh, auto_inc_service_type_id, table_name, column_1_name, column_1_value, column_2_name, column_2_value):
	
		
	try:
		#sql ="SELECT * FROM %s" % (tableName)
		sql ="SELECT %s FROM %s WHERE %s=%s AND %s=%s" % (auto_inc_service_type_id, table_name, column_1_name, column_1_value,column_2_name,column_2_value)
